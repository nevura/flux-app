...
import { notifLabel as notifLabelShared } from '@/lib/notificationCopy'

function notifLabel(n: Notification): { icon: string; iconColor: string; text: string } {
  return notifLabelShared(n.type, n.data as Record<string, unknown>)
}
    ...
    ;(async () => {
      ...
      realtimeChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            const newNotif = payload.new as import('@/lib/types').Notification
            setUnread(prev => prev + 1)
            setList(prev => {
              if (prev.length === 0) return prev // panel closed — update on open
              return [newNotif, ...prev]
            })
            const { text } = notifLabelShared(newNotif.type, newNotif.data as Record<string, unknown>)
            toast(text)
          }
        )
        .subscribe()
